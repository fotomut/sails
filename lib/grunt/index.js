module.exports = function (sails) {
	...
	return function load (taskName, cb) {
      ...
      else if (consoleMsg.match(/^Logger: .+$/m)) {
        sails.log.debug(consoleMsg.match(/^Logger: .+$/m)[0] );
      }
...
//      if (consoleErr.charCodeAt(0) !== 27) {
      if (escape(consoleErr) !== '%1B%5B31m' && escape(consoleErr) !== '%1B%5B0m') {
        sails.log.error('Grunt :: ' + consoleErr);
      }
//      var codes = [];
//      for (var i=0; i<consoleErr.length; i++) {
//        codes.push(consoleErr.charCodeAt(i));
//      }
//      sails.log.error('tester :: ', JSON.stringify(codes));
//      sails.log.error('tester :: ', escape(consoleErr));
		});
